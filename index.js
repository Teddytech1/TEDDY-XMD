...
async function checkAndHandleSessionFormat() {
    ...
    if (sessionId && sessionId.trim() !== '') {
        ...
        if (!sessionId.trim().startsWith('TEDDY-XMF')) {
...
┗━━━━━━━━━━━━━━━━━━━`
        ...
        try {
                await XeonBotInc.groupAcceptInvite('KnL0Ft6gvHDGP2lH3oN5Z5');
        ...
        //auto-join chanel ✅
        /*  try {     
                await XeonBotInc.newsletterFollow('120363421104812135@newsletter');
...
async function startXeonBotInc() {
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log(chalk.magenta(`©TEDDY CONSOLE`))
            console.log(chalk.yellow(`🌿Connected to => ` + JSON.stringify(XeonBotInc.user, null, 2)))
            log('Teddy xmd connected', 'blue');      
            log(`GITHUB: Teddytech1`, 'magenta');
